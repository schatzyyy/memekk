		...
		case 'halloweentext':
		    ...
		    break
				...
				case 'afk':
                                        ...
                                        kev.sendMessage(from, tag, text, {quoted: mek})
                                        break
				case 'pantaitext':				
                 if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
...
*/
				...
				break
				case 'iniapa':
				kev.sendMessage(from, `hai`, text, { quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "documentMessage": { "title":"i love you","mimetype": "image/jpeg", "caption": "i love you tu", 'jpegThumbnail': fs.readFileSync('./assets/halo.jpg')}}}})
				break
				case 'addlimit':
                  if (isBanned) return reply(ind.baned())				
				if (!isRegistered) return reply(ind.noregis())
...
tels = body.slice(6)
try {
anu = await fetchJson(`https://tobz-api.herokuapp.com/api/wiki?q=${tels}&apikey=${TobzKey}`, {
  method: 'get'
})
reply(anu.result)
